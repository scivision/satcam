...
def iridium_ncdf(fn: Path, day, tlim, ellim, camlla):
    assert len(ellim) == 2, 'must specify elevation limits'
    fn = Path(fn).expanduser()
    # %% get all sats psuedo SV number
    with Dataset(str(fn), 'r') as f:
        # psv_border = nonzero(diff(f['pseudo_sv_num'])!=0)[0]
        # didn't work because of consecutively reused psv
        # unique doesn't work because psv's can be recycled
        psv_border = (diff(f['time']) < 0).nonzero()[
            0
        ] + 1  # note unequal number of samples per satellite, +1 for how diff() is defined
        # %% iterate over psv, but remember different number of time samples for each sv.
        # since we are only interested in one satellite at a time,
        # why not just iterate one by one, throwing away uninteresting results
...
def iridium_tle(fn, T, sitella, svn):
    ...
    # %% comp sat position
    obs = Observer()
    obs.lat = str(sitella[0])
    obs.lon = str(sitella[1])
    obs.elevation = float(sitella[2])
